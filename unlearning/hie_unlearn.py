...
class Coach:
    ...
    def run(self):
        ...
        for ep in range(stloc, args.epoch):
            if ep % args.sim_epoch == 0:
                ...
                mi_result = self.test_unlearn(self.model, prefix='HIE test:')
    ...
    def prepare_model(self):
        ...
        self.model = HIE(self.handler, trained_model, ini_embeds.detach(), fnl_embeds.detach()).cuda()
    ...
    def trn_epoch(self):
        ...
        for i, tem in enumerate(trn_loader):
            ...
            log('Step %d/%d: loss = %.6f, regLoss = %.6f, unlearn = %.6f, align = %.6f         '
                % (i, steps, loss.item(), reg_loss.item(), unlearn_loss.item(), align_loss.item()),
