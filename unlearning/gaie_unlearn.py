...
class Coach:
    ...
    def prepare_model(self):
        ...
        self.model = GAIE(self.handler, trained_model, ini_embeds.detach(), fnl_embeds.detach()).cuda()
    ...
    def trn_epoch(self):
        ...
        ep_loss, ep_preloss, ep_unlearn_loss, ep_align_loss, ep_rec_loss = [0] * 5
        ...
        for i, tem in enumerate(trn_loader):
            ...
            if t.isnan(loss).any() or t.isinf(loss).any():
                _comps = {
                    ...
                    'align': align_loss, 'rec': rec_loss,
                }
                _bad = {k: v.item() for k, v in _comps.items()
                        if t.isnan(v).any() or t.isinf(v).any()}
                print(f'\n[gaie] NaN/Inf at step {i}/{steps}: loss={loss.item()}  bad={_bad}')
                self.opt.zero_grad()
                continue
        ...
        ret['rec_loss'] = ep_rec_loss / steps
        return ret
